...
BOOL GHid::Open(WORD Pid_Open, WORD Vid_Open)
{
	Close();
	...
	PSP_DEVICE_INTERFACE_DETAIL_DATA	pDevDetailData;					//定义一个用来保存打开设备的句柄。//定义一个HIDD_ATTRIBUTES的结构体变量，保存设备的属性。
	...
	{		
		...
		Result=SetupDiGetDeviceInterfaceDetail(hDevInfoSet,
												...
												&RequiredSize,
												NULL);	
		pDevDetailData=(PSP_DEVICE_INTERFACE_DETAIL_DATA)malloc(RequiredSize);//然后，分配一个大小为RequiredSize缓冲区，用来保存设备详细信息。
		...
		pDevDetailData->cbSize=sizeof(SP_DEVICE_INTERFACE_DETAIL_DATA);		//并设置pDevDetailData的cbSize为结构体的大小（注意只是结构体大小，不包括后面缓冲区）。
